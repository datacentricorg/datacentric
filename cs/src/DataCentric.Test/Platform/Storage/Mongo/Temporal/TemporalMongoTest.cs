    ...
    public class TemporalMongoTest
    {
        ...
        public void ElementTypesQuery()
        {
            using (var context = new TemporalMongoTestContext(this))
            {
                ...
                var testQuery = context.GetQuery<DerivedSampleData>(dataSet1)
                    ...
                    .Where(p => p.InstantElement < new LocalDateTime(2003, 5, 1, 10, 15, 01).ToInstant())
                    .Where(p => p.InstantElement > new LocalDateTime(2003, 5, 1, 10, 14, 59).ToInstant())
                    .Where(p => p.InstantElement == new LocalDateTime(2003, 5, 1, 10, 15).ToInstant())
        ...
        public void SavedBy()
        {
            using (var context = new TemporalMongoTestContext(this))
            {
                ...
                context.DataSource.CastTo<TemporalMongoDataSourceData>().SavedById = cutoffRecordId;
        ...
        private RecordId SaveBaseRecord(IContext context, string dataSetName, string recordName, int recordIndex)
        {
            ...
            rec.InstantElement = new LocalDateTime(2003, 5, 1, 10, 15).ToInstant(); // 2003-05-01T10:15:00
